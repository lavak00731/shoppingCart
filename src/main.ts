...
const elems =  async () => {
  ...
  const prods = []
  if(request.length > 0) {
    document.querySelector<HTMLElement>('#app')!.innerHTML = `<ul id="list" class="grid grid-cols-2 md:grid-cols-4 lg:flex lg:overflow-x-auto lg:space-x-4 lg:w-7/8 lg:mx-auto"></ul>`;
